...
def process_timelapse(job_id, frigate_url, camera, start_ts, end_ts, speed, fps, source_export_id=None):
    ...
    try:
        ...
        if not export_id:
            ...
            # 1. Trigger Export
            export_url = f"{frigate_url}/api/export/{camera}/start/{start_ts}/end/{end_ts}"
            logger.info(f"Job {job_id}: Requesting export from {export_url}")
            
            if stop_event.is_set(): raise Exception("Cancelled")

            # Trigger export
            r_export = requests.post(export_url, json={}, timeout=30)
            ...
            logger.info(f"Job {job_id}: Export started (ID: {export_id}). Waiting for completion...")
        else:
             ...
             jobs[job_id]['status'] = 'exporting' # Re-using this status to mean "waiting for export to be ready"
        ...
        while time.time() - start_wait < 600: # 10 min timeout
            if stop_event.is_set(): raise Exception("Cancelled")
            try:
                r_status = requests.get(f"{frigate_url}/api/exports", timeout=10)
        ...
        jobs[job_id]['filename'] = f"timelapse_{camera if camera else 'export'}_{speed}x.mp4"
        ...
        if should_delete_export:
             try:
                ...
                requests.delete(f"{frigate_url}/api/export/{export_id}", timeout=10)
             except Exception as e:
        ...
        logger.error(f"Job {job_id} {'cancelled' if is_cancelled else 'failed'}: {str(e)}")
...
def get_cameras():
    ...
    try:
        ...
        resp = requests.get(f"{url}/api/config", timeout=10)
...
def get_exports():
    ...
    url = data.get('url', '').rstrip('/')
    try:
        resp = requests.get(f"{url}/api/exports", timeout=10)
        resp.raise_for_status()
        exports = resp.json()
...
def create_timelapse():
    ...
    thread = threading.Thread(target=process_timelapse, args=(
        job_id, 
        data['frigateUrl'], 
        data.get('camera'), 
        start_ts, 
